...
*/
    ...
    public function imprimir(Request $request){
        ...
        $fecha_letra = $this->fechaLetra($fecha);
        ...
        $pdf = \PDF::loadView('imprimir', compact('eventos','fecha','nombre_alcalde','fecha_letra'));
        return $pdf->download('agenda-'. $fecha . '.pdf');
        }
        ...
        if($request->opcion == 1){
            ...
            $semana['inicio'] = $this->fechaAbrev($dias[0]);
            $semana['fin'] = $this->fechaAbrev($dias[count($dias)-1]);

            foreach($dias as $dia){
                $semana['dias'][$dia]['eventos'] = Evento::whereDate('fecha_inicio', $dia)->orderBy('fecha_inicio','ASC');

                if($request->tipo==1){
                     $semana['dias'][$dia]['eventos']->where('atiende_alcalde', 1);
                }
                if($request->tipo==2){
                     $semana['dias'][$dia]['eventos']->where('atiende_alcalde', 0);
                }
                $semana['dias'][$dia]['eventos'] = $semana['dias'][$dia]['eventos']->get();
                $semana['dias'][$dia]['fecha_abreviada'] = $this->fechaAbrev($dia); 
            }
            ...
            $pdf = \PDF::loadView('imprimir-semana', compact('semana','fecha','nombre_alcalde','fecha_letra'));
            return $pdf->download('agenda-semanal-'. $fecha . '.pdf');
